      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold text-gray-900">Saved Blogs ({blogs.length})</h1>
            <Button
          ...
          {blogs.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">No blogs saved yet</h3>
              <p className="text-gray-600">Generate and save your first blog post to see it here.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {blogs.map((blog) => (
                <div key={blog.id} className="bg-gray-50 rounded-lg p-6 border border-gray-200 hover:shadow-md transition-shadow">
                  ...
                  <div className="space-y-1 mb-4 text-sm text-gray-600">
                    <p><strong>Keyword:</strong> {blog.primaryKeyword}</p>
                    <p><strong>Word Count:</strong> {blog.wordCount}</p>
                    <p><strong>Created:</strong> {new Date(blog.createdAt).toLocaleDateString()}</p>
                    {blog.targetAudience && <p><strong>Audience:</strong> {blog.targetAudience}</p>}
                    {blog.toneOfVoice && <p><strong>Tone:</strong> {blog.toneOfVoice}</p>}
                  </div>
        ...
        {showDetails && selectedBlog && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <h2 className="text-xl font-bold text-gray-900">{selectedBlog.title}</h2>
                  <Button
