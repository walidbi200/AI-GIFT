...
const BlogPost: React.FC<BlogPostProps> = ({ 
  ...
  const processContent = (content: string) => {
    return content
      ...
      .replace(/^# (.*$)/gim, '<h1 class="text-4xl font-bold text-gray-900 mt-12 mb-6">$1</h1>')
      .replace(/\n- (.*$)/gim, '<br>• $1')
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <head>
        ...
        <link rel="canonical" href={`https://smartgiftfinder.xyz/blog/${blog.slug}`} />
        
        {/* Open Graph */}
        <meta property="og:title" content={blog.title} />
        <meta property="og:description" content={blog.description} />
        <meta property="og:image" content={blog.featuredImage} />
        <meta property="og:url" content={`https://smartgiftfinder.xyz/blog/${blog.slug}`} />
              ...
              {relatedPosts.slice(0, 3).map((post) => (
                ...
                >
                  ...
                  <div className="mt-3 text-xs text-gray-500">
                    {formatDate(post.createdAt)} • {Math.ceil(post.wordCount / 200)} min read
                  </div>
                </a>
              ))}
            </div>
